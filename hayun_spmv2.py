...
class Clustering(nn.Module):
    ...
    def forward(self):
        ...
        reduced_y = reduced_y.reshape(C, int(K/4), 4)
        reduced_y = 1. - torch.prod(1 - reduced_y, 2)
        y = torch.sum(y, dim=[0, 1])
        y = y.reshape(int(K/4), 4)
        y = y.sum(axis=1) / 4
        return torch.sum(reduced_y, dim=0), y  # [K/4], [K/4]
...
target_col = torch.zeros((int(K/4))).cuda()
#target_col.data.fill_(C/K)
target_col.data.fill_(2000)
target_calc = torch.zeros((int(K/4))).cuda()
target_calc.data.fill_(R*C*0.25*4/K)
...
for i in range(1000):
    ...
    #loss = criterion(col_per_cluster, target_col)*1000 + criterion(calc_per_cluster, target_calc)
    #loss = criterion(col_per_cluster, target_col)
