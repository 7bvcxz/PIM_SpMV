...
col = torch.count_nonzero(fc, dim=0) * 1.0
...
for row_i in range(num_row):
    if nnz_k[k_i] + nnz[row_i] > nnz_per_cluster:
        ...
        col_k[k_i] = torch.logical_or(col_k[k_i], fc[row_i])*2
        row_k[k_i] = row_k[k_i] + 1
        k_i = k_i + 1
        nnz_k[k_i] = nnz_k[k_i] + (nnz[row_i] - front_nnz)
        col_k[k_i] = torch.logical_or(col_k[k_i], fc[row_i])*1 # just doing for now
        row_k[k_i] = row_k[k_i] + 1
    else:
        nnz_k[k_i] = nnz_k[k_i] + nnz[row_i]
        col_k[k_i] = torch.logical_or(col_k[k_i], fc[row_i])*1
        row_k[k_i] = row_k[k_i] + 1
...
#print("col_k : ", col_k)
#print("row_k : ", row_k)
print_data(nnz_k, "nnz_k")
print_data(col_k, "col_k")
print_data(row_k, "row_k")
